...
async def fetch_gas_prices(id):
    ...
    gasbuddy._load_cookies("./data/cookies/my_cookies.json")
    max_retries = 5
...
async def search_by_zipcode(zip_code):
    ...
    gasbuddy = GasBuddy(cookie_file="./data/cookies/my_cookies.json")
...
async def get_station_by_coor(lat, lon):
    ...
    gasbuddy = GasBuddy()
    gasbuddy.load_cookies_from_file("./data/cookies/my_cookies.json")
    response = await gasbuddy.location_search(lat, lon)
    if "error" in response:
        print("Error:", response["error"])
        return {"error": response["error"]}  # <--- never return None
    else:
