...
def convert_lowercase(filename: str, output_file: str = None):
    """Read Excel, convert ISO times to PDT, and optionally save back."""
    df = pd.read_excel(filename)
    logging.info(f"Loaded {len(df)} rows from {filename}")
    ...
    logging.info("filtering and changing to Uppercase.")
...
# Example usage
# df_updated = convert_excel_times("data/gas_prices.xlsx", "data/gas_prices_pdt.xlsx")
df_updated = convert_lowercase("data/gas_prices.xlsx", "data/gas_prices_pdt.xlsx")
